...
def create_kits(body, token_info):
    ...
    # NB: The barcodes_container variable is a list of dicts, structured with
    # the notion that each element of the list contains data for a "sample
    # slot" in the set of kits being created. Each dict contains a boolean
    # variable named 'barcodes_provided' which reflects whether the barcodes
    # for this sample slot are admin-provided (True) or are to be generated by
    # the system (False). If 'barcodes_provided' is True, 'barcodes' will be a
    # list of barcodes. If 'barcodes_provided' is False, 'barcodes' will be
    # an empty list.
    barcodes_container = body['barcodes_container']
    ...
    with Transaction() as t:
        ...
        for sample_slot in barcodes_container:
            ...
            if sample_slot['barcodes_provided'] is True:
                ...
                if len(sample_slot['barcodes']) != number_of_kits:
                    errors.append(
                        f'Incorrect number of barcodes provided for Sample {x}'
                    )

                # And verify that the barcodes don't already exist
                for barcode in sample_slot['barcodes']:
                    diag = admin_repo.check_exists_barcode(barcode)
                    if diag is True:
                        errors.append(f'Barcode {barcode} already exists')
            else:
                # Barcodes list should be empty in this sample slot's dict
                if len(sample_slot['barcodes']) != 0:
                    errors.append(
                        f'Barcodes erroneously provided for Sample {x}'
                    )
            x += 1
        ...
        try:
            ...
            return jsonify(
                code=422,
                message="Unable to create kits: " + str(ke)
            ), 422
        except RepoException as re:
            return jsonify(
                code=422,
                message="Unable to create kits: " + str(re)
            ), 422
        else:
            t.commit()
...
def add_barcodes_to_kits(body, token_info):
    ...
    with Transaction() as t:
        ...
        try:
            ...
            return jsonify(
                code=422,
                message="Unable to add barcodes to kits: " + str(ke)
            ), 422
        else:
            t.commit()
